...
class _OrdersScreenState extends State<OrdersScreen> {
  ...
  Future<void> _loadOrders() async {
    try {
      final List<FreegiOrder> savedOrders =
          await OrderStorage.getOrders();

      final List<Map<String, dynamic>> convertedOrders =
          savedOrders.map(_orderToMap).toList();
    ...
    if (historyOrders.isEmpty) {
      ScaffoldMessenger.of(context)
        ...
        ..showSnackBar(
          SnackBar(
            content: const Text(
              'No delivered or cancelled orders to clear.',
            ),
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
        ...
        ..showSnackBar(
          SnackBar(
            content: const Text(
              'Unable to clear order history right now.',
            ),
            behavior: SnackBarBehavior.floating,
            ...
            const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 2),
                  Text(
                    'Track your orders, relive your purchases',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      color: subText,
                      fontSize: 10,
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: _statItem(
              value:
                  '${orders.where((o) => o['type'] == 'delivered').length}',
              label: 'Delivered',
              icon: Icons
          ...
          Expanded(
            child: _statItem(
              value:
                  '${orders.where((o) => o['type'] == 'active').length}',
              label: 'Active',
              icon: Icons
                  .inventory_2_rounded,
            ),
          ...
          Expanded(
            child: _statItem(
              value:
                  '${orders.where((o) => o['type'] == 'cancelled').length}',
              label: 'Cancelled',
              icon: Icons
                  .cancel_rounded,
            ),
